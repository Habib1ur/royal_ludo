...
export function MoveHistory({ entries, performanceMode = false }: MoveHistoryProps) {
  return (
    <section className={`rounded-[2rem] border p-5 ${performanceMode ? 'border-white/10 bg-slate-900/90 shadow-[0_8px_18px_rgba(15,23,42,0.16)]' : 'border-white/15 bg-white/10 shadow-glass backdrop-blur-xl'}`}>
      ...
      <div className="max-h-[22rem] space-y-3 overflow-auto pr-1">
        {entries.length === 0 ? (
          <div className={`rounded-2xl border p-4 text-sm text-slate-300 ${performanceMode ? 'border-white/8 bg-slate-950/72' : 'border-dashed border-white/15'}`}>
            Rolls, captures, and wins show up here.
          </div>
        ) : (
          entries.map((entry) => (
            <div key={entry.id} className={`rounded-2xl border p-3 text-sm ${performanceMode ? 'border-white/8 bg-slate-950/72' : 'border-white/10 bg-slate-950/30'}`}>
